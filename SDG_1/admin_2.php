            ...
            if (isset($_POST['approvalstat'])) {
              if($_POST['approvalstat']=='Approved'){
                ...
                if($_POST['approvalcert']=='Approved'){
                  mysqli_query($conn,"UPDATE internship_data SET completion_status = 'Completed' WHERE internship_data.id = '$id'");
                  header('location:admin_1.php');
                }else{
                  echo '<script>alert("Rejected");</script>';
                  mysqli_query($conn,"UPDATE internship_data SET approval_status = '".$_POST['approvalcert']."',completion_status = 'Rejected' WHERE internship_data.id = '$id'");
                  header('location:admin_1.php');
                }
              }
            ?>
